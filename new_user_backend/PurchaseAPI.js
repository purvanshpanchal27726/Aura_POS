...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const masterQuery = `
      INSERT INTO purchase_master (
        vendor_id, purchase_date, purchase_bill_no, gross, tax, total, created_by
      ) VALUES (?, ?, ?, ?, ?, ?, ?)
    `;
...
/**
 * GET /api/purchase
 * Fetches all purchase master records with vendor details, sorted by purchase_id ascending.
 */
 ...
 * Fetches all purchase detail lines with date, vendor, company, user, category, and item name.
 */
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const [masterRows] = await db.execute(masterQuery, [purchaseId]);
 ...
 * Deletes a purchase invoice (cascades to details) and shifts subsequent IDs down.
 */
router.delete('/:id', async (req, res) => {
  try {
    const purchaseId = parseInt(req.params.id);
    const [rows] = await db.execute('SELECT * FROM purchase_master WHERE purchase_id = ?', [purchaseId]);
    if (rows.length === 0) {
      return res.status(404).json({ error: 'Purchase invoice not found' });
    }

    await db.query('START TRANSACTION');
    await db.execute('DELETE FROM purchase_master WHERE purchase_id = ?', [purchaseId]);
    await db.execute('UPDATE purchase_master SET purchase_id = purchase_id - 1 WHERE purchase_id > ?', [purchaseId]);
    await db.execute('ALTER TABLE purchase_master AUTO_INCREMENT = 1');
