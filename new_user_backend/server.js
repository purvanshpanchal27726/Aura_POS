...
app.get('/api/dashboard/stats', async (req, res) => {
  try {
    ...
    let lowStockWhere = cidFilter ? `${cidFilter} AND (stock_quantity <= 10)` : ' WHERE (stock_quantity <= 10)';
    ...
    let lowStockListQuery = `
      SELECT item_id, name, item_code, COALESCE(stock_quantity, 0) AS quantity 
      FROM items
      ${lowStockWhere}
